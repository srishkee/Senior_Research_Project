...
def pretty_print(FOLDER, PLANT_COLOR, IMG_IDX, imgs, max_pred_idx, max_label, max_pred, predictions):
	...
	tmp = []
	tmp = (predictions)
	sum = np.ndarray.sum(tmp)
	#print("sum: ", sum)
	mean = sum/38
	#print("mean: ", mean)
	ans = "PREDICTION: Category " + str(max_pred_idx) + ", '" + max_label +  "' with an accuracy = " + str((max_pred-mean)*10) + "%\n"
	print(ans)
...
PRETRAINED = '/scratch/user/skumar55/mohanty/AWS_FRESH_RUN/snapshots_final/googLeNet_segmented-80-20_finetune/snapshots__iter_54407.caffemodel'
